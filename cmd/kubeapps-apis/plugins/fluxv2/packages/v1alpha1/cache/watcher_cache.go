...
func (c *NamespacedResourceWatcherCache) Watch(options metav1.ListOptions) (watch.Interface, error) {
	...
	if err != nil {
		return nil, connect.NewError(connect.CodeFailedPrecondition, fmt.Errorf("Unable to get client due to: %w", err)) //nolint:staticcheck
	}
...
func (c *NamespacedResourceWatcherCache) resync(bootstrap bool) (string, error) {
	...
	if !common.RWMutexWriteLocked(c.resyncCond.L.(*sync.RWMutex)) {
		return "", connect.NewError(connect.CodeInternal, fmt.Errorf("Invalid state of the cache in resync()")) //nolint:staticcheck
	}
	...
	if !bootstrap {
		...
		if err := c.config.OnResyncFunc(); err != nil {
			return "", connect.NewError(connect.CodeInternal, fmt.Errorf("Invocation of [OnResync] failed due to: %w", err)) //nolint:staticcheck
		}
	}
	...
	ctrlClient, err := c.config.ClientGetter.ControllerRuntime(ctx)
	if err != nil {
		return "", connect.NewError(connect.CodeFailedPrecondition, fmt.Errorf("Unable to get client due to: %w", err)) //nolint:staticcheck
	}
...
func (c *NamespacedResourceWatcherCache) syncHandler(key string) error {
	...
	if err != nil {
		...
		if errors.IsNotFound(err) {
			...
			return connect.NewError(connect.CodeInternal, fmt.Errorf("Error fetching object with key [%s]: %w", key, err)) //nolint:staticcheck
		}
	}
...
func (c *NamespacedResourceWatcherCache) populateWith(items []ctrlclient.Object) error {
	...
	// of resync, i.e. resync.Cond.L is locked by this goroutine.
	if !common.RWMutexWriteLocked(c.resyncCond.L.(*sync.RWMutex)) {
		return connect.NewError(connect.CodeInternal, fmt.Errorf("Invalid state of the cache in populateWith()")) //nolint:staticcheck
	}
...
func (c *NamespacedResourceWatcherCache) NamespacedNameFromKey(key string) (*types.NamespacedName, error) {
	...
	if len(parts) != 3 || parts[0] != c.config.Gvr.Resource || len(parts[1]) == 0 || len(parts[2]) == 0 {
		return nil, connect.NewError(connect.CodeInternal, fmt.Errorf("Invalid key [%s]", key)) //nolint:staticcheck
	}
